...
golintmu/
  ...
  cmd/golintmu/
    main.go                     # singlechecker.Main(Analyzer)
  golintmu.go                   # Analyzer definition, passContext, run()
  lockstate.go                  # lockState: COW fork, lock/unlock, isHeld
  lockstate_test.go
  resolver.go                   # lockRef resolution from SSA values
  resolver_test.go
  ssawalk.go                    # SSA CFG walker, instruction dispatch
  ssawalk_test.go
  inference.go                  # Guard inference algorithm
  inference_test.go
  interprocedural.go            # Bottom-up requirement propagation, call graph
  interprocedural_test.go
  concurrency.go                # Concurrent entrypoint detection
  concurrency_test.go
  facts.go                      # Fact types, export/import
  facts_test.go
  annotations.go                # Comment directive parsing
  annotations_test.go
  reporter.go                   # Diagnostic formatting, call chain display
  reporter_test.go
  golintmu_test.go              # Integration tests using analysistest
  testdata/src/
    basic/                      # Simple lock/unlock patterns
    inference/                  # Inference voting scenarios
    interprocedural/            # Cross-function lock propagation
    concurrent/                 # Entrypoint detection
    rwmutex/                    # RWMutex read vs write
    crosspackage/pkga/pkgb/     # Cross-package facts
    embedded/                   # Embedded mutex
    multimutex/                 # Structs with multiple mutexes
    defer_patterns/             # defer mu.Unlock() patterns
    false_positives/            # Must NOT fire: constructors, init, immutable fields
    annotations/                # //mu: directives
    double_lock/                # C2: double locking
```

## 8. Dependencies

```
module github.com/akerouanton/golintmu
...
Only dependency: `golang.org/x/tools` for:
- `golang.org/x/tools/go/analysis` — Analyzer framework
- `golang.org/x/tools/go/analysis/passes/buildssa` — SSA construction
- `golang.org/x/tools/go/ssa` — SSA types
- `golang.org/x/tools/go/analysis/singlechecker` — CLI harness
...
- Lock ordering (C3) via lock-order graph + cycle detection
